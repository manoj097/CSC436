...
const Product = class Product {
    constructor(input) {
        ...
        this.name = name.trim();
        this.price = parseFloat(price.trim());
     ...
     */
     static halfOff = (products) => {
      const productmap = products.map(product => new Product(`${product.name},${product.price * 0.5},${product.availability}`));
      return productmap;
      }
     ...
     */
     static printProducts = (products) => {
        ...
        products.forEach(product => {
          const changedPrice = formatter.format(product.price);
          const avail = product.availability === 'In Stock' ? 'Yes' : 'No';
          console.log(`Product: ${product.name}, Cost: ${changedPrice}, Availability: ${avail}`);
        });
      }
